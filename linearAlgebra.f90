...
! which solves Ãx=b or Ã^Tx=b for some preconditioner matrix Â
...
select case(itol)	! Calculate norms for use in stopping criterion,
	...
	case(2)			! is z(1:n); the final 0 indicates that the
		call asolve(b,z,0) 	! matrix Ã (not its transpose) is to 
...
do		! Main loop
	...
	bknum=dot_product(z,rr)	! matrix Ã^T. Calculate coefficient
	...
	r=r-ak*z
	rr=rr-ak*zz
	call asolve(r,z,0)	! Solve Ãz=r and check stopping criterion
	select case(itol)
		case(1)
...
call sprsdiag(sa,x)
! The matrix Ã is taken to be the diagonal part of A. Since the 
